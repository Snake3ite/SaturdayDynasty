...
// Sends through a Supabase Edge Function so delivery does not depend on third-party form activation.
(()=>{
...
#sdfFbStatus{display:none;padding:10px 11px;border:1px solid #29495a;border-radius:8px;background:#0a1d29;color:#a9c1ce;font-size:11px;line-height:1.45}#sdfFbStatus.show{display:block}#sdfFbStatus.good{border-color:#286b58;color:#79e6c8}#sdfFbStatus.bad{border-color:#7b3541;color:#ff9cac}
...
`;
  ...
  function make(){
    ...
    const btn=document.createElement('button');btn.id='sdfFeedbackBtn';btn.type='button';btn.setAttribute('aria-label','Send feedback or report a bug');btn.innerHTML='<span aria-hidden="true">🐞</span><span>FEEDBACK</span>';btn.onclick=open;document.body.appendChild(btn);
    ...
    modal.innerHTML=`
      <section id="sdfFeedbackBox" role="dialog" aria-modal="true" aria-labelledby="sdfFeedbackTitle">
        ...
        <form id="sdfFeedbackForm" enctype="multipart/form-data">
          <input type="hidden" name="Page" id="sdfFbPage"><input type="hidden" name="Browser" id="sdfFbBrowser"><input type="hidden" name="SubmittedAt" id="sdfFbTime"><input class="sdfFbHoney" type="text" name="website" tabindex="-1" autocomplete="off">
          <div class="sdfFbGrid">
          ...
          <label class="sdfFbField"><span>WHAT HAPPENED?</span><textarea name="Description" required minlength="3" maxlength="5000" placeholder="Tell us what you were doing, what you expected, and what went wrong."></textarea></label>
          <label class="sdfFbField sdfFbFile"><span>SCREENSHOT (OPTIONAL)</span><input id="sdfFbAttachment" type="file" name="attachment" accept="image/png,image/jpeg,image/webp"><small class="sdfFbHint">PNG, JPG, or WEBP. Maximum 4 MB.</small></label>
    ...
    const form=modal.querySelector('#sdfFeedbackForm');const file=form.querySelector('#sdfFbAttachment');
    ...
    file.addEventListener('change',()=>{if(file.files?.[0]?.size>MAX_FILE_BYTES){file.value='';setStatus('That screenshot is over 4 MB. Please choose a smaller image.','bad')}else resetStatus()});

    form.addEventListener('submit',async e=>{
      e.preventDefault();
      const f=file.files?.[0];if(f&&f.size>MAX_FILE_BYTES)return setStatus('That screenshot is over 4 MB. Please choose a smaller image.','bad');
      const c=window.SDF_CLOUD_CONFIG;if(!c?.supabaseUrl||!c?.publishableKey)return setStatus('Feedback service configuration is missing.','bad');

      form.querySelector('#sdfFbPage').value=location.href;form.querySelector('#sdfFbBrowser').value=navigator.userAgent;form.querySelector('#sdfFbTime').value=new Date().toISOString();
      const send=form.querySelector('.sdfFbSend');send.disabled=true;send.textContent='SENDING…';setStatus('Sending your feedback…');
      const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),20000);
      try{
        const r=await fetch(`${c.supabaseUrl}/functions/v1/send-feedback`,{method:'POST',headers:{apikey:c.publishableKey,Accept:'application/json'},body:new FormData(form),signal:controller.signal});
        let data=null;try{data=await r.json()}catch{}
        if(!r.ok||data?.ok!==true)throw new Error(data?.error||`Feedback service returned ${r.status}.`);
        setStatus('Feedback sent. Thank you!','good');form.querySelector('textarea[name="Description"]').value='';file.value='';
      }catch(err){
        const msg=err?.name==='AbortError'?'The feedback request timed out.':(err?.message||'Unknown submission error.');setStatus(`Feedback was NOT sent. ${msg}`,'bad');
      }finally{clearTimeout(timer);send.disabled=false;send.textContent='SEND FEEDBACK'}
    });
  ...
  const init=()=>make();document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init):init();window.SDF_FEEDBACK={open,close};
})();
