...
(()=>{
 ...
 function queuePut(slot,raw){const prior=pending.get(slot)||Promise.resolve();const next=prior.catch(()=>{}).then(()=>idbPut(slot,raw)).then(()=>{const key=keyFrom(slot);try{rawRemove.call(localStorage,key)}catch{}}).catch(reportFailure);pending.set(slot,next);return next}
 function queueDelete(slot){const prior=pending.get(slot)||Promise.resolve();const next=prior.catch(()=>{}).then(()=>idbDelete(slot)).catch(reportFailure);pending.set(slot,next);return next}
 ...
 Storage.prototype.removeItem=function(key){const slot=isLocal(this)?slotFrom(key):null;if(slot){const k=String(key);cache.delete(k);try{rawRemove.call(this,k)}catch{};queueDelete(slot);return}return rawRemove.call(this,key)};
