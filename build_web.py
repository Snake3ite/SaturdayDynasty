...
html=html.replace('Android V27.4.36','Web V27.4.36').replace('href="manifest.json"','href="manifest.webmanifest"')
html=re.sub(r'<script src="(?:ad-config|app-bundle)\.js[^\"]*"></script>','',html)
html=html.replace('</head>','<link rel="stylesheet" href="browser-shell.css?v=237">\n</head>')
html=html.replace('</body>','<script src="browser-bootstrap.js?v=237"></script>\n</body>')
...
sw="""const CACHE='sdf-web-build237';
const CORE=__CORE__;
...
(OUT/'WEB_BUILD_VERIFICATION.json').write_text(json.dumps({'versionCode':237,'version':'27.4.36','files':checks},indent=2),encoding='utf-8',newline='')
print('Built Web237: shared runtime preserved, browser integrations included,',len(checks),'assets verified.')
