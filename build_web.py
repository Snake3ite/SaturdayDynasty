...
html=html.replace('Android V27.4.48','Web V27.4.48').replace('href="manifest.json"','href="manifest.webmanifest"')
html=re.sub(r'<script src="(?:ad-config|app-bundle)\.js[^\"]*"></script>','',html)
html=html.replace('</head>','<link rel="stylesheet" href="browser-shell.css?v=249">\n</head>')
html=html.replace('</body>','<script src="browser-bootstrap.js?v=249"></script>\n</body>')
...
sw="""const CACHE='sdf-web-build249';
const CORE=__CORE__;
...
(OUT/'WEB_BUILD_VERIFICATION.json').write_text(json.dumps({'versionCode':249,'version':'27.4.48','files':checks},indent=2),encoding='utf-8',newline='')
print('Built Web249: shared runtime preserved, browser integrations included,',len(checks),'assets verified.')
