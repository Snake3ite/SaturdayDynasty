...
alter table sdf_private.usage_devices add column if not exists retention_started_at timestamptz;
alter table sdf_private.usage_devices alter column retention_started_at set default now();
alter table sdf_private.usage_devices add column if not exists returned_day1 boolean not null default false;
alter table sdf_private.usage_devices add column if not exists returned_day7 boolean not null default false;
...
begin
 ...
 for e in select value from jsonb_array_elements(p_events) loop
  ...
  if inserted=1 and p_device is not null and not p_qa then
   update sdf_private.usage_devices set
    returned_day1=returned_day1 or (stamp>=retention_started_at+interval '24 hours' and stamp<retention_started_at+interval '48 hours'),
    returned_day7=returned_day7 or (stamp>=retention_started_at+interval '168 hours' and stamp<retention_started_at+interval '192 hours')
   where device=p_device and retention_started_at is not null;
  end if;
...
begin
 ...
 if report is null then
  ...
  report=report||jsonb_build_object('retention',coalesce((select jsonb_agg(x) from (
   select platform,
    count(*) filter(where retention_started_at<=cutoff-interval '48 hours') as eligible1,
    count(*) filter(where retention_started_at<=cutoff-interval '48 hours' and returned_day1) as returned1,
    count(*) filter(where retention_started_at<=cutoff-interval '192 hours') as eligible7,
    count(*) filter(where retention_started_at<=cutoff-interval '192 hours' and returned_day7) as returned7
   from sdf_private.usage_devices where not qa and retention_started_at is not null group by platform
  ) x),'[]'::jsonb));
